...
int inf_gain(vector<vector<string> > data, set<int> used,set<int>randomnumbers,set<int>randomrows){ //information gain calculation
    bool flag = false;
    int index = -1;
...
void ID3(attribute **root,vector<vector<string> >data,set<int>used){   //TREE 
...
int a,b,c,d,e,f = 0,g = 0;
 ...
 //CREATION       
    ...
    attribute *root;
    set<int>temp;
    cout<<endl<<endl<<"Please wait for around 15 seconds"<<endl;      
      for(int i = 0; i<5; i++){
        root = new attribute();
        set<int>temp;
...
for(int i = 0; i<testingdata.size();i++){   //TESTING
	...
	total++;
    if(test[i][0]>test[i][1])ans = 0; else ans = 1;
